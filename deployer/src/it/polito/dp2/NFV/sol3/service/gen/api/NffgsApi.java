...
public class NffgsApi {
	...
	@PUT
	@Consumes({ "application/xml" })
	@io.swagger.annotations.ApiOperation(value = "deploys a NF-FG", notes = "Deploys a new NF-FG into the system (when a new NF-FG is deployed, its nodes are allocated onto the IN hosts; if it is not possible to allocate all the NF-FG nodes, deployment fails, and nothing is added). If deployment is successful, deploy time is set to the current time.", response = String.class, tags = {
			"nffgs", })
	@io.swagger.annotations.ApiResponses(value = {
			@io.swagger.annotations.ApiResponse(code = 201, message = "(Resource) Created", response = String.class),
